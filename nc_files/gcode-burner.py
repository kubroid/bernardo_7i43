 ...
 #                         G_CALLBACK (gtk_widget_destroy),
  #                        dialog);
	...
	def generate_gcode(self, arg):
		...
		l = len(pixels)
		ch = int((l-self.h*2)/self.w/self.h)
		if ch not in [1,2,3,4] : 
			ch = self.pixbuf.get_n_channels() # number of color chanels
		...
		for j in range(self.h):
			...
			if self.checkbuttons['echo_filter_progress'].get_active():
				print "FILTER_PROGRESS=%d" % int(100*j/self.h)
			for i in range(self.w):
				r = ord(pixels[j*(self.w*ch+2)+i*ch])
				g = ord(pixels[j*(self.w*ch+2)+i*ch+1])
				b = ord(pixels[j*(self.w*ch+2)+i*ch+2])
				v = float(r+g+b)/255/3
				v_sum += 1.-v
